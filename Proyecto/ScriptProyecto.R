# Script Proyecto Muestreo y Planificación de las Encuestas I
# Integrantes del grupo: Valentina Rodríguez Blanco, Macarena Perdomo, Sol Astengo.
...
mse_SI <- V_empirica_SI + (sesgo_empirico_SI)**2
...
mse_SIR <- V_empirica_SIR + (sesgo_empirico_SIR)**2
...
correlaciones <- sapply(datos, function(x) {
  if (is.numeric(x))
    cor(datos$HOGCE13, x, use = "complete.obs")
  else
    NA
})

correlaciones <- correlaciones[!is.na(correlaciones)]

correlaciones <- sort(correlaciones, decreasing = TRUE)

correlaciones

cor(datos$HOGHD00, datos$HOGCE13) #Se elegie una variable con mayor correlación con la variable 
# objeto de estudio
...
mse_ST <- V_empirica_ST + (sesgo_empirico_ST)**2
...
mse_CONG <- V_empirica_CONG - + (sesgo_empirico_CONG)**2
...
#=========== COEFICIENTE DE HOMOGENEIDAD INTRACLASE ============

# Calculamos la media de HOGCE13 dentro de cada conglomerado
datos_icc <- datos %>%
  group_by(codsegm) %>%
  mutate(media_cong = mean(HOGCE13)) %>%
  ungroup()

# Cantidad total de hogares y conglomerados
N_icc <- nrow(datos_icc)
NI_icc <- length(unique(datos_icc$codsegm))

# Suma de cuadrados total
SST <- sum((datos_icc$HOGCE13 - mean(datos_icc$HOGCE13))^2)

# Suma de cuadrados dentro de conglomerados
SSW <- sum((datos_icc$HOGCE13 - datos_icc$media_cong)^2)

# Varianza total y varianza dentro de conglomerados
S2_total <- SST / (N_icc - 1)
S2_within <- SSW / (N_icc - NI_icc)

# Coeficiente de homogeneidad intraclase
delta <- 1 - (S2_within / S2_total)

delta
SST
SSW

# Segun el delta positivo que tiende a 0, y la similitud entre SST y SSW,
# se concluye que hay muy poca homogeneidad intra cluster, que es lo ideal en
# un diseño eficiente por conglomerado, por lo tanto la alta variabilidad del
# diseño no se explica por falta de heterogeneidad dentro de los conglomerados.
...
# Los resultados de la simulación muestran que los cuatro estimadores presentan 
# un sesgo empírico muy reducido respecto al verdadero total poblacional.
# La comparación de las varianzas empíricas y de los errores cuadráticos medios 
# permitió identificar al diseño estratificado como el más eficiente en términos de variabilidad entre los 
# considerados, teniendo además el único efecto diseño menor a 1 del 0.924.

# El diseño SIR presentó un comportamiento muy similar al del diseño SI. Tanto la 
# varianza, como el error cuadrático medio, fueron levemente superiores a los 
# obtenidos bajo diseño aleatorio simple, lo que indica que, para esta población 
# y este tamaño muestral, el muestreo con reemplazo no aporta mejoras en términos de precisión.

#El diseño por conglomerados resultó ser el menos eficiente teniendo incluso una varianza cien 
#veces mas grande que la de un diseño SI.
...
## Conclusiones ...
#Los intervalos empiricos son bastante similares a los intervalos normales y se concentran 
#al rededor del total poblacional, a excepción del diseño por conglomerados que presenta un 
#intervalo amplio y mucho menos preciso (en comparación al estimador bajo los otros diseños)
...
df_cong_alt <- data.frame(sim_cong_alt = sim_cong_alt)

dist_cong_alt <- ggplot(df_cong_alt, aes(x = est_cong)) +
  geom_histogram(bins = 40, fill = "aquamarine3", color = "white") +
  geom_vline(xintercept = total, color = "red", linetype = "dashed", linewidth = 1) +
  geom_vline(xintercept = mean(est_cong), color = "blue", linewidth = 1) +
  labs(
    title = "Distribución empírica del estimador π alternativo bajo diseño por conglomerados",
    subtitle = "UPM: segmentos censales",
    x = expression(hat(t)[pi]),
    y = "Frecuencia"
  ) +
...
# Conclusiones 
#Con la simulación de la distribución del estimador alternativo se logró dismunuir la varianza
#(aunque aumentó levemente el sesgo) que también resultó en intervalos de confianza más precisos 
#en comparación a la distribución del estimador horvitz thompson, además presenta menos diferencia
#entre los extremos de los intervalos empíricos y los calculados asumiendo normalidad.